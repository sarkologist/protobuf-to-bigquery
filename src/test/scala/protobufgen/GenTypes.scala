...
object GenTypes {
  ...
  private val genInt32  = Gen.choose[Int](-1, 1)
  private val genInt64  = Gen.choose[Long](-1, 1)
  private val genUInt64 = Gen.chooseNum[Long](0, 1)
  private val genUInt32 = Gen.chooseNum[Int](0, 1)
  val smallFloat        = Gen.choose[Float](-1, 1)
  val smallDouble       = Gen.choose[Double](-1, 1)
  ...
  val ProtoDouble =
    Primitive("double", smallDouble.map(_.toString))
  val ProtoFloat =
    Primitive("float", smallFloat.map(_.toString))
  val ProtoBool =
    Primitive("bool", Arbitrary.arbitrary[Boolean].map(_.toString))
  ...
  val ProtoBytes =
    Primitive("bytes",
              Gen.listOf(Arbitrary.arbitrary[Byte]).map(escapeBytes),
              packable = false)
  ...
  case class MapType(keyType: ProtoType, valueType: ProtoType)
      extends ProtoType {
    def packable = false
    def isMap    = true
  }
  ...
  def genFieldModifier(allowRequired: Boolean): Gen[FieldModifier.Value] =
    if (allowRequired)
      Gen.oneOf(FieldModifier.OPTIONAL,
  ...
  def genFieldType(
    ...
    val withMessages =
      if (state._nextMessageId > 0 && allowCurrentMessage)
        (10, Gen.chooseNum(0, state._nextMessageId - 1).map(MessageReference)) :: baseFreq
      else if (!allowCurrentMessage && state.currentFileInitialMessageId > 0)
        (1,
         Gen
           .chooseNum(0, state.currentFileInitialMessageId - 1)
           .map(MessageReference)) :: baseFreq
      else baseFreq
  ...
  def genOptionsForField(
    ...
    if (inOneof)
      Gen.const(FieldOptions(FieldModifier.OPTIONAL, isPacked = false))
    else
      fieldType match {
        case MessageReference(id) =>
          genFieldModifier(
            allowRequired = protoSyntax.isProto2 && id < messageId)
            .map(mod => FieldOptions(mod, isPacked = false))
        case MapType(_, _) =>
          Gen.const(FieldOptions(FieldModifier.REPEATED, isPacked = false))
        case _ =>
          for {
