...
object Nodes {
  ...
  private def snakeCaseToCamelCase(name: String,
                                   upperInitial: Boolean): String = {
  ...
  case class RootNode(files: Seq[FileNode]) {
    ...
    def javaClassName(m: MessageNode) = {
      ...
      val innerClassName =
        (if (file.protoPackage.isDefined) parts.tail else parts).mkString("$")
    ...
    lazy val messagesById: Map[Int, MessageNode] =
      files.flatMap(_.allMessages).map(m => (m.id, m)).toMap
    lazy val enumsById: Map[Int, EnumNode] =
      files.flatMap(_.allEnums).map(e => (e.id, e)).toMap
  ...
  final case class ServiceNode(name: String, methods: Seq[MethodNode]) {
    def print(printer: FunctionalPrinter): FunctionalPrinter =
      printer
        .add(s"service $name {")
        .indent
        .print(methods)((node, p) => p print node)
        .outdent
        .add("}")
  }
  ...
  case class FileNode(
    ...
    def allMessages =
      messages.foldLeft(Stream.empty[MessageNode])(_ ++ _.allMessages)
    ...
    def fileReferences(rootNode: RootNode): Set[String] =
      (for {
        message <- allMessages
        ...
        .collect({
          ...
          case MapType(keyType @ _, EnumReference(id)) =>
            rootNode.enumsById(id).fileId
          case MapType(keyType @ _, MessageReference(id)) =>
            rootNode.messagesById(id).fileId
        })
        .toSet
        .map(rootNode.filesById)
        .map(_.baseFileName)

    def print(rootNode: RootNode,
              printer: compiler.FunctionalPrinter): compiler.FunctionalPrinter =
      printer
        .add(
          s"// File id $fileId. messages: $minMessageId-$maxMessageId. Enums: $minEnumId-$maxEnumId"
        )
        .add(s"""syntax = "${protoSyntax.syntaxName}";""")
        .print(protoPackage)((p, pkg) => p.add(s"package $pkg;"))
        .print(javaPackage)((p, pkg) =>
          p.add(s"""option java_package = "$pkg";"""))
    ...
    def javaOuterClassOrPackage: Either[String, String] = {
      ...
      if (javaMultipleFiles) {
        ...
        Left(
          (pkg :+ snakeCaseToCamelCase(baseFileName, upperInitial = true)) mkString ".")
      }
    }
  ...
  case class MessageNode(
    ...
    def allMessages: Stream[MessageNode] =
      messages.foldLeft(Stream(this))(_ ++ _.allMessages)

    def allEnums: Stream[EnumNode] =
      messages.foldLeft(enums.toStream)(_ ++ _.allEnums)
    ...
    def print(
      ...
      def makeList(fields: Seq[FieldNode]): Seq[FieldLine] = {
        def makeList0(fields: Seq[FieldNode],
                      prevGroup: Option[String]): Seq[FieldLine] =
          if (fields.isEmpty) Seq.empty
          else
            ((fields.head.oneOfGroup, prevGroup) match {
              case (OneofContainer(name), Some(otherName))
                  if (name != otherName) =>
              ...
              makeList0(
                fields.tail,
                if (fields.head.oneOfGroup.isOneof)
                  Some(fields.head.oneOfGroup.name)
                else None
              )
      ...
      printer
        ...
        .indent
        .print(enums)((enum, p) => p.print(enum))
        .print(messages)((p, message) => message.print(rootNode, fileNode, p))
  ...
  case class FieldNode(
    ...
    def print(
      ...
      if (!oneOfGroup.isOneof) {
        ...
        printer.add(
          s"${modifier}${rootNode.resolveProtoTypeName(fieldType)} $name = $tag$packed;  // $fieldType"
        )
      } else {
        printer.add(
          s"${rootNode.resolveProtoTypeName(fieldType)} $name = $tag;  // $fieldType")
      }
  }
