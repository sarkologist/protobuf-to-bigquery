...
import protobufgen.GenTypes.{FieldOptions, ProtoType}
import protobufgen.GenUtils._
...
object GraphGen {
  ...
  case class Namespace(names: Set[String], parent: Option[Namespace]) {
    ...
    def isNameAvailable(name: String): Boolean =
      !names.contains(name.toLowerCase) && parent.forall(
        _.isNameAvailable(name))

    def generateName: Gen[String] =
      SchemaGenerators.identifier.retryUntil(isNameAvailable)

    // See https://github.com/google/protobuf/issues/2738
    def generateJavaPackageName: Gen[String] =
      SchemaGenerators.identifier.retryUntil(s =>
        s != "key" && isNameAvailable(s))
  }
  ...
  case class State(
    ...
    def nextMessageId =
      (_nextMessageId, copy(_nextMessageId = _nextMessageId + 1))
    ...
    def nextEnumId(syntax: ProtoSyntax, zeroDefined: Boolean) =
      (
        ...
        copy(
          ...
          proto3EnumIds =
            if (syntax.isProto3) proto3EnumIds :+ _nextEnumId
            else proto3EnumIds,
          enumsWithZeroDefined =
            if (zeroDefined) enumsWithZeroDefined :+ _nextEnumId
            else enumsWithZeroDefined
        )
      )
    ...
    def generateJavaPackageName: Gen[(String, State)] =
      namespace.generateJavaPackageName.map { s =>
        (s, this.copy(namespace = namespace.add(s)))
      }
    ...
    def closeNamespace: State =
      namespace.parent.fold(
        throw new IllegalStateException("Attempt to close root namespace"))(p =>
        copy(namespace = p))
  }
  ...
  def genEnumNode(parentMessageId: Option[Int], syntax: ProtoSyntax)(
    ...
    for {
      zeroDefined <- if (syntax.isProto3) Gen.const(true)
      else Gen.oneOf(false, true)
      (enumName, state) <- state.generateName
      (myId, state)     <- Gen.const(state.nextEnumId(syntax, zeroDefined))
      (names, state) <- GenUtils
        .listWithStatefulGen(state, minSize = 1, maxSize = 2)(_.generateName)
        .retryUntil {
          case (names, _) =>
            ...
            names.forall(n =>
              !n.toLowerCase.replaceAll("_", "").startsWith(enumNameCanon)) &&
  ...
  def genOneOfs(fieldCount: Int,
                state: State): Gen[(List[OneOfGrouping], State)] = {
    def genBits(
        n: Int,
        seqSize: Int,
        prev: OneOfGrouping,
      ...
      if (n == 0)(Gen.const((Nil, state)))
        ...
        Gen.frequency(
          (2, genBits(n - 1, 0, NotInOneof, state).map {
  ...
  def genMessageNode(depth: Int = 0,
                     parentMessageId: Option[Int] = None,
    ...
    sized { s =>
      for {
        ...
        (messages, state) <- listWithStatefulGen(state,
                                                 maxSize = (2 - depth) max 0)(
          genMessageNode(depth + 1, Some(myId), protoSyntax)
        )
        (enums, state) <- listWithStatefulGen(state, maxSize = 1)(
          genEnumNode(Some(myId), protoSyntax)
        )
        fieldCount <- choose[Int](0, s min 2)
        (fieldNames, state) <- listOfNWithStatefulGen(fieldCount, state)(
          _.generateName)
        ...
        fieldTypes <- Gen.sequence[Seq[ProtoType], ProtoType](
          isInOneof.map(isOneof =>
            GenTypes.genFieldType(state, protoSyntax, allowMaps = !isOneof))
        )
        fieldOptions <- Gen.sequence[Seq[FieldOptions], FieldOptions](
          (fieldTypes zip isInOneof).map {
            case (fieldType, inOneof) =>
              GenTypes.genOptionsForField(myId,
                                          fieldType,
                                          protoSyntax,
                                          inOneof = inOneof)
          }
  ...
  def genService(messages: Seq[MessageNode])(
      state: State): Gen[(ServiceNode, State)] =
    for {
  ...
  def genMethod(messages: Seq[MessageNode])(
      state: State): Gen[(MethodNode, State)] =
    for {
  ...
  def genFileNode(state: State): Gen[(FileNode, State)] =
    for {
      ...
      (javaPackageNames, state) <- GenUtils.listWithStatefulGen(state,
                                                                minSize = 1,
                                                                maxSize = 1)(
      ...
      (protoPackage, state) <- Gen.oneOf(state.generateSubspace,
                                         Gen.const(("", state)))
      protoPackageOption = if (protoPackage.nonEmpty) Some(protoPackage)
      else None
      (messages, state) <- listWithStatefulGen(state, maxSize = 2)(
        genMessageNode(0, None, protoSyntax)
      )
      (enums, state) <- listWithStatefulGen(state, maxSize = 1)(
        genEnumNode(None, protoSyntax))
      javaMulti <- implicitly[Arbitrary[Boolean]].arbitrary
      (services, state) <- listWithStatefulGen(state, maxSize = 0)(
        genService(messages))
    } yield
      (
        FileNode(
  ...
  def genRootNode: Gen[RootNode] =
    listWithStatefulGen(State(), maxSize = 1)(genFileNode)
      .map {
        case (files, state) =>
